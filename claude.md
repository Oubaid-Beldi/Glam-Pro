...
- Known issues / TODO:
  - **Post-deploy hotfix, same session:** `/api/generate-posts` returned live `502`s after deploy — root cause was Groq deprecating `llama-3.3-70b-versatile` (confirmed via a live `/openai/v1/models` check), unrelated to the refactor itself. Fixed by switching `GROQ_MODEL` (in `server/integrations/groq.client.ts`) to `openai/gpt-oss-120b`, verified against the real production prompt before landing. See the addendum in docs/sessions/07-architecture-refactor.md. Still **not** re-verified with a live authenticated round-trip through the actual Marketing UI post-fix-deploy (would need an interactive Google sign-in handoff) — do this first at the start of session 8.
  - **`SUPABASE_SERVICE_ROLE_KEY` was rotated this session** after an `netlify env:list --context production` call printed it unmasked (that var had never been flagged "secret" in Netlify — only `GROQ_API_KEY` was). New key set via `netlify env:set ... --secret` for `production` (rejected for `dev` context — Netlify CLI disallows `--secret` there, so `dev`'s copy is set but unflagged). See the addendum in docs/sessions/07-architecture-refactor.md for the full incident and the `netlify dev:exec`-over-`env:list` lesson for future sessions.
