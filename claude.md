...
- **Voice input (as of session 8):** speech-to-text for the AI-generation objective field, via Groq Whisper (`whisper-large-v3-turbo`) — same `GROQ_API_KEY`, no new provider or account. Browser records audio with MediaRecorder, sends it base64-encoded to a backend endpoint, backend transcribes it and returns plain text that fills the objective field for the user to review/edit before generating — voice never skips straight to AI generation.
...
- Known issues / TODO:
  - **Voice input's actual browser-mic round trip on the live site is not yet confirmed by a real recording** — see docs/sessions/08-voice-input.md's verification section. The deployed endpoint, auth gate, and the real Groq Whisper call (via an isolated test with a real speech clip) are all confirmed working; only the live "click mic, speak, see the objective field fill in" step needs a manual check, since it couldn't be automated in this environment (no real mic available to the browser tooling used, and reading the live session token to script it was correctly blocked as a credential-extraction action). Do this first at the start of session 9 if not already done.
