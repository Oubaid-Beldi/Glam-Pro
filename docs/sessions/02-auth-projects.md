...
- **Two pre-existing Supabase misconfigurations, not introduced this session, caused most of the session's time:**
  ...
  - **Action for the user:** revoke the Supabase Personal Access Token used for this debugging (dashboard → Account → Access Tokens) — it grants full account-level Management API access and was only needed for this session, not for ongoing use. The Netlify CLI login can stay linked; it's the normal path for env var / deploy work.
...
2. Confirm with the user that the temporary Supabase Personal Access Token from this session's debugging has been revoked (dashboard → Account → Access Tokens) — don't assume it, ask.
3. Build `tasks` CRUD scoped to the active project — status/priority as colored badges, simple filterable list, no drag-and-drop Kanban — per `docs/PLAN.md` Day 3. No schema changes needed; the `tasks` table + RLS policy already exist from session 1's `supabase/schema.sql`.
4. Reuse `useProjects()`'s `activeProject` to scope all task queries/inserts (same pattern `projects-context.tsx` establishes). If there's no active project yet, the Tasks page should prompt the user to create/select one first, mirroring `Projects.tsx`'s empty state.
5. Deploy and update this session log / CLAUDE.md status again at the end, per the standard session-close convention. Plain `git push`-triggered auto-deploy should be sufficient again (no CLI env var surgery expected).
