...
## Addendum: post-deploy hotfix — Groq model deprecated

After the refactor deployed, the user reported `/api/generate-posts` failing live with `502 Bad Gateway` from the Marketing page. Root-caused and fixed in the same session, before close-out:

- **Not a refactor regression.** A live `GET https://api.groq.com/openai/v1/models` check (via `netlify dev:exec`, which injects the real project env vars into a local Node script without ever printing the API key) showed `llama-3.3-70b-versatile` is gone entirely from Groq's model catalog — calling it now returns `404 model_not_found`. That 404 is a non-`429`, non-ok HTTP response, which `posts.service.ts` correctly maps to `provider_unreachable` → `502 "AI generation failed. Please try again."` — the exact error the user saw. The refactor moved this logic verbatim; the model itself was decommissioned by Groq independently, some time between session 6's verification and now.
- **Fix:** queried the live `/models` endpoint for what's currently available, then re-ran the exact production system/user prompt (with `response_format: json_object`) against the most comparable candidate, `openai/gpt-oss-120b` — confirmed `200 OK` with well-formed `{"drafts": [...]}` JSON. Updated `GROQ_MODEL` in `server/integrations/groq.client.ts` (now the single source of truth for the model name, a direct benefit of this session's centralization) and CLAUDE.md's stack line. No other code changed — this is a one-constant fix, not a prompt or logic change.
- **CLAUDE.md's "don't deviate from the stack without a strong reason, record why" applies here**: the model changed because Groq removed the old one from their catalog, not by choice. If it happens again, `netlify dev:exec node <script that hits /openai/v1/models>` is the fastest way to check what Groq currently serves before guessing a replacement name.

**Credential-handling incident during this diagnosis, worth a future session heeding:** an early diagnostic step (`netlify env:list --context production`) printed `SUPABASE_SERVICE_ROLE_KEY` unmasked into the conversation, because that var had never been flagged "secret" in Netlify (only `GROQ_API_KEY` was). Flagged to the user immediately; they provided a newly rotated key on the spot, which was set via `netlify env:set SUPABASE_SERVICE_ROLE_KEY <value> --context production --secret` (and `--context dev`, unflagged — Netlify CLI rejects `--secret` for the `dev` context). **Lesson for future sessions:** prefer `netlify dev:exec` (injects env vars into a subprocess without ever printing them) over `netlify env:list`/`env:get` when the goal is just to use a var's value, not inspect it — and if `env:list` output is ever needed, check the table for unmasked values before assuming Netlify's masking covers every sensitive var. Non-secret-looking vars (a Supabase project URL, a publishable anon key) are fine to see in full; anything server-role/admin-scoped is not.
